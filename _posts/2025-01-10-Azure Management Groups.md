...
flowchart LR
  U[Usuarios Internet] --> DNS[DNS Público]
  DNS --> FE[App Service (Linux+Docker)\nFrontend Angular SSR + NGINX]
  FE -->|HTTPS| API[App Service (Linux+Docker)\nAPI NestJS]

  subgraph SUB[Suscripción PRE o PRO (idéntico por entorno)]
    direction LR

    subgraph NET[Spoke VNet (ALZ)]
      direction TB
      SN1[snet-apps]
      SN2[snet-data]
      SN3[snet-privatelinks]
    end

    FE --- SN1
    API --- SN1

    API -->|MI| KV[Azure Key Vault\nSecretos]
    API -->|SDK/Private Endpoint| COSMOS[Azure Cosmos DB\nStudentPortalDB]
    API -->|Private Endpoint| REDIS[Azure Cache for Redis]

    API -->|Client Credentials| GRAPH[Microsoft Graph] --> SPO[SharePoint Online]

    FE --> APPIF[App Insights Front]
    API --> APPIA[App Insights API]
    APPIF --> LAW[Log Analytics Workspace]
    APPIA --> LAW

    ENTRA[Microsoft Entra ID\nAppReg Front + AppReg API] --> FE
    ENTRA --> API
  end


flowchart LR
  U[Usuarios Internet] --> AFD[Azure Front Door\nCDN + WAF + TLS]
  AFD --> FE[App Service (Linux+Docker)\nFrontend Angular SSR + NGINX]
  FE --> APIM[API Management\nPolicies + Rate limit + Versioning]
  APIM --> API[App Service (Linux+Docker)\nAPI NestJS]
  ...
  subgraph SUB[Suscripción PRE o PRO (idéntico por entorno)]
    ...
    APIM -. opcional VNet .- SN1

    API -->|MI| KV[Azure Key Vault]
    API -->|Private Endpoint| COSMOS[Azure Cosmos DB]
    API -->|Private Endpoint| REDIS[Azure Cache for Redis]
    API -->|Client Credentials| GRAPH[Microsoft Graph] --> SPO[SharePoint Online]
